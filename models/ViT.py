...
default_cfgs = {
    ...
    ################### ViT-tiny ###################
    'vit_tiny_patch16_224.augreg_in21k_ft_in1k': _cfg(  #(ft=finetune)
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.):
        ...
        self.rand_idx = torch.randperm(dim)

  
    def forward(self, x, opt_cd):
        B, N, C = x.shape # torch.Size([60, 197, 768])

        qkv = F.linear(input=x, weight=self.qkv.weight + 0.1 * self.pd_weight(self.qkv.weight) , bias=self.qkv.bias)
        ...
        x = F.linear(input=x, weight=self.proj.weight + self.theta_msa * self.mpd_weight(self.proj.weight) , bias=self.proj.bias)
    ...
    def rotate_all(self, w):
        # return torch.cat((w[:, 1:] , w[:, :1]), 1) # 1: -->
        # return torch.cat((w[:, w.shape[1]-1:] , w[:, :w.shape[1]-1]), 1) # 2: <-- 
        # return torch.cat((w[:, 1:] , w[:, :1]), 1) + torch.cat((w[:, w.shape[1]-1:] , w[:, :w.shape[1]-1]), 1) # 3: <-- & -->
        return w[:,self.rand_idx] # 4: random
...
class Block(nn.Module):
    def __init__(
            ...
            drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, opt_cd=False):
        super().__init__()
    ...
    def forward(self, x):
        ...
        x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x), opt_cd=self.opt_cd)))    
...
class VisionTransformer(nn.Module):
    def __init__(
            ...
            weight_init='', embed_layer=PatchEmbed, norm_layer=None, act_layer=None, block_fn=Block, opt_cd=False):
        """
        Args:
        ...
        self.myblock = block_fn
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, depth)]  # stochastic depth decay rule
        self.blocks = nn.Sequential(*[
            block_fn(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, init_values=init_values,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer, opt_cd=opt_cd)
    ...
    def forward_features(self, x, opt_cd): 
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            # self.norm = norm_layer(768) if not 'avg' else nn.Identity()
            # dpr = [x.item() for x in torch.linspace(0, 0., 12)]  # stochastic depth decay rule
            # self.blocks = nn.Sequential(*[self.myblock(
            #         dim=768, num_heads=12, mlp_ratio=4., qkv_bias=True, init_values=None,
            #         drop=0., attn_drop=0., drop_path=dpr[i], norm_layer=norm_layer, act_layer=nn.GELU, opt_cd=opt_cd)
            #     for i in range(12)]).cuda()

            x = self.blocks(x) # norm_layer -> Attentson -> LayerScale -> DropPath
        x = self.norm(x)
        return x
    ...
    def forward(self, x, train=True, opt_cd=False):
        # if train == True:
        ...
        #     return x, binary_map
        # else:
        x = self.forward_features(x, opt_cd=opt_cd)
        x = self.forward_head(x)
        return x
...
################################ ViT-B/16 ################################
@register_model
def vit_base_patch16_224(pretrained=False, opt_cd=False, **kwargs):
    """ ViT-Base (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 224x224, source https://github.com/google-research/vision_transformer.
    # """
    # drop_rate=0., attn_drop_rate=0.,
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12,
                        drop_rate=0.0, attn_drop_rate=0.0, opt_cd=opt_cd, **kwargs)
...
if __name__ == "__main__":
    ...
    # Fas_Net(x) 
    flops, params = profile(Fas_Net, inputs=(x,))
