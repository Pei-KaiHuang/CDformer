...
from .ViT import Block as TransformerBlock
...
class Mobilemain(nn.Module):
    def __init__(self):
        super().__init__()
        net = mobilevit_s(pretrained=True)
        self.rgbA = net.stem
        self.rgbB = net.stages
        
        net = mobilevit_s(pretrained=False, num_classes=2)
        self.Identity = net.final_conv
        self.classification = net.head
    def forward(self, R):
        R = self.rgbA(R)
        R = self.rgbB(R)
...
if __name__ == "__main__":
    ...
    print(net(x,x,x).shape)
    # print(net.state_dict().values())